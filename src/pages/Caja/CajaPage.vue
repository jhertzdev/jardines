  ...
  <q-page>
        ...
        <div class="q-pa-md">
          ...
          <div class="flex justify-between q-mb-md">
            <div class="q-gutter-xs">
              ...
              <q-btn dense color="negative" :flat="!verAnulados" label="Ver anulados" icon="archive"
                @click="verAnulados = true; tipoTransaccion = null, transaccionesTableRef.requestServerInteraction()" />
            </div>
            <div class="q-gutter-sm">
              <q-btn dense class="q-px-sm" color="primary" label="Crear recibo" icon="receipt" to="/caja/recibos/nuevo"
                v-if="!authStore.user.role_perms.find((role) => role == 'cajas.verFiscal')" />
              <q-btn dense class="q-px-sm" color="primary" label="Relación de ingresos" icon="assessment"
                to="/caja/reportes/ingresos" />
              <q-btn dense class="q-px-sm" color="primary" label="Histórico" icon="timeline"
                @click="openDialogHistorialIngresos()"
                v-if="authStore.user.role_perms.find((role) => role == 'cajas.*' || role == 'cajas.verFiscal')" />
              <q-btn dense class="q-px-sm" color="primary" label="Reporte" icon="article"
                @click="handleGenerarReporteGeneral()"
                v-if="authStore.user.role_perms.find((role) => role == 'cajas.*' || role == 'cajas.verFiscal')" />
            </div>
          </div>

          <q-table dense :rows="transacciones" :columns="transaccionesColumnas" row-key="name"
            :class="!$q.screen.lt.md && 'text-wrap'" ref="transaccionesTableRef"
            v-model:pagination="transaccionesTablePagination" :loading="transaccionesTableLoading"
            :filter="transaccionesTableFilter" @request="transaccionesTableRequest">
            <template v-slot:top-right>
              <q-btn dense icon="search" color="primary" label="Buscar pagos" class="q-pr-sm q-mr-md"
                @click="showDialogBuscarPagos = true" />
            ...
            <template v-slot:body-cell-fiscal="props">
              <q-td :props="props" style="padding-right: 0;" :class="props.row.deleted_at && 'highlighted'">
                <q-checkbox v-model="props.row.es_fiscal" dense size="sm" color="primary" class="q-mr-xs" true-value="1"
                  false-value="0" @update:model-value="handleSetEstatusFiscal(props.row.id, props.row.es_fiscal)"
            ...
            <template v-slot:body-cell-num_contrato="props">
              <q-td :props="props">
                <a href="javascript:void(0)"
                  @click="verContratosDialog.openDialog(props.row.contrato.num_contrato, props.row.contrato.tipo_parcela)">
            ...
            <template v-slot:body-cell-acciones="props">
              <q-td :props="props" class="q-gutter-x-xs">
                <q-btn color="primary" size="sm" dense icon="print" :loading="isLoadingPrint"
                  @click="handleDownloadPdf(props.row.id, props.row.contrato)" :disable="props.row.estatus !== 'Pagado'"
                  ...
                  ">
                    ...
                    <q-icon
                      v-else-if="new Date(props.row.contrato.vigente_hasta) >= add(props.row.contrato.fecha_vencimiento, { years: 1 })"
                      name="warning" color="yellow-2">
                      <q-tooltip max-width="200px" class="text-center bg-black">
                ...
                <q-btn color="primary" size="sm" dense icon="credit_card"
                  @click="openDialogAgregarPago(props.row.id)" />
                <template v-if="!props.row.deleted_at">
                  <span>|</span>
                  <q-btn color="negative" size="sm" dense icon="archive"
                    @click="handleVerificarEliminarTransaccion(props.row.id)">
                    <q-tooltip class="bg-black">Anular transacción</q-tooltip>
                  </q-btn>
                  <q-btn color="primary" size="sm" dense icon="check"
                    @click="handleEditarTransaccion(props.row.id, { estatus: 'Pagado' })"
                    v-if="props.row.estatus != 'Pagado'">
                    <q-tooltip class="bg-black">Marcar como pagado</q-tooltip>
                  </q-btn>
                  <q-btn color="warning" size="sm" dense icon="timer"
                    @click="handleEditarTransaccion(props.row.id, { estatus: 'Pendiente' })"
        ...
        <q-card-section class="q-pa-none">
          <div class="row">
            <div class="col-12 col-md-5 q-py-lg">
              ...
              <q-list separator bordered>
                <q-item v-for="item in transaccionData?.lineas || []">
                  ...
                  <q-item-section side class="text-right">
                    ...
                    <div class="text-caption">{{ transaccionData?.simbolo }} {{ $dinero(item.total_ref *
                      (transaccionData?.tasa_actual || 1)) }}</div>
                  </q-item-section>
                </q-item>
              </q-list>
              <q-table flat square bordered hide-bottom hide-header :columns="totalsColumns" :rows="totalsData"
                ...
                <template v-slot:body="props">
                  <q-tr :props="props" class="text-right"
                    :class="props.rowIndex + 1 == totalsData.length && 'bg-primary text-white'"
                    v-show="!!props.row.total">
                    ...
                    <q-td key="total" :props="props" class="text-right">
                      <div class="q-pl-xl" :class="props.rowIndex + 1 == totalsData.length && 'text-h5'">{{
                        $dinero(props.row.total) }}</div>
                      <div class="text-grey">{{ $dinero(props.row.total * (transaccionData?.tasa_actual || 1)) }} {{
                        transaccionData?.simbolo }}</div>
                    </q-td>
                  </q-tr>
                </template>
              </q-table>
              ...
              <div class="q-pl-md">
                <q-input type="textarea" outlined v-model="transaccionData.descripcion" rows="4"
                  label="Descripción del recibo"
                  placeholder="Escribe una descripción para el recibo, que se mostrará en la versión impresa"
                  readonly />
              </div>
            </div>
            <div class="col-12 col-md-7 q-pa-lg text-center">
              ...
              <div class="q-my-lg">
                <div class="text-h3">
                  <span class="text-body1 text-grey-9">{{ transaccionData?.simbolo }}</span> {{ $dinero(totalBalance *
                    (transaccionData?.tasa_actual || 1)) }}
                </div>
                <div class="text-h6">
              ...
              <q-table flat bordered hide-bottom hide-header class="q-mt-md" selection="multiple"
                v-model:selected="metodosPagoSelected" :rows="metodosPago" :columns="metodosPagoColumnas"
                :pagination="{ rowsPerPage: -1 }">
                <template v-slot:body-cell-cantidad="props">
                  <q-td :props="props" class="text-center" style="padding-bottom: .75rem"
                    :style="!metodosPagoSelected.includes(props.row) && 'white-space: break-spaces;'">
                    ...
                    <template v-if="metodosPagoSelected.includes(props.row)">
                      ...
                      <template
                        v-if="monedaPrincipal.id === props.row.moneda_id && mostrarAvisoIgtf && !ignorarMostrarAvisoIgtf">
                        <div>
                          ...
                          <q-input dense v-model="calcularIgtfInput" type="number" step="0.01" label=""
                            hint="Escribe un monto en divisas para calcular el IGTF">
                            <template v-slot:append>
                              <q-btn size="sm" dense class="q-px-sm" color="primary" icon="calculate" label="Calcular"
                                @click="handleAgregarLineaIgtf(transaccionData.id)"></q-btn>
                            </template>
                          </q-input>
                          <q-checkbox dense class="q-mt-xs" v-model="ignorarMostrarAvisoIgtf"
                            label="Ignorar aviso de IGTF para esta transacción" />
                        </div>
                      </template>

                      <template v-else>
                        <q-input dense v-model="props.row[props.col.name]" type="number" step="0.01"
                        ...
                        <q-input dense v-model="props.row['referencia']" label="Referencia"
                          v-if="props.row['tipo_metodo'] != 'Efectivo'" debounce="500"
                          @update:model-value="val => handleVerificarReferencia(props.row, val)"
                          :error="props.row.referencias_usadas?.length > 0 && !props.row['referencia_usada']"
                          hide-bottom-space>
                          <template v-slot:error>
                            La referencia ya ha sido usada.
                          </template>
                        </q-input>
                        <q-checkbox dense class="q-mt-xs" v-if="props.row.referencias_usadas?.length > 0"
                          :model-value="!!props.row['referencia_usada']"
                          @update:model-value="props.row['referencia_usada'] = !props.row['referencia_usada']"
                          label="Utilizar referencia igualmente" />
                      </template>
      ...
      <q-card class="q-pa-md">
        ...
        <q-card-section>
          <div class="flex items-center justify-end">
            <q-input class="q-mr-sm" label="Reporte del mes" dense flat v-model="filterMonth" mask="####-##"
              :hide-bottom-space="true" outlined readonly clearable style="width: 180px; display: inline-flex">
              <template v-slot:append>
                <q-icon v-if="filterMonth" name="close" class="cursor-pointer" @click="filterMonth = ''"></q-icon>
                <q-icon name="event" class="cursor-pointer">
                  <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                    <q-date v-model="filterMonth" default-view="Months" emit-immediately
                      v-close-popup="filterDateClosePopup" @update:model-value="filterDateClosePopup = true"
                      @navigation="filterDateClosePopup = false"
                      :default-year-month="(filterMonth || new Date().toISOString().substr(0, 7)).replace('-', '/')"
                      years-in-month-view>
            ...
            <q-btn color="primary" label="Ver reporte mensual" icon="print"
              @click="handleDescargarReporteIngresosMensual(filterMonth)" :disable="filterMonth == ''" />
          </div>
          <q-markup-table flat>
            <thead>
              <tr>
            ...
            <tbody>
              <tr v-for="cierre in cajaHistorial" :key="cierre.id">
                ...
                <td style="max-width: 150px; line-height: 1.15; white-space: break-spaces">{{ cierre.nombre_completo }}
                  ({{
                    cierre.username }})</td>
                <td class="q-gutter-x-xs">
                  <template v-if="authStore.user.role_perms.find((role) => role == 'cajas.*')">
                    <q-btn color="primary" size="sm" class="q-px-sm" label="General" icon="print"
                      @click="handleDescargarReportePorCierre(cierre.id)" />
                    <q-btn color="primary" size="sm" class="q-px-sm" label="Por usuario" icon="print"
                      @click="handleDescargarReportePorCierre(cierre.id, 'por_usuario')" />
                    <q-btn color="primary" size="sm" class="q-px-sm" label="Fiscal" icon="print"
                      @click="handleDescargarReportePorCierre(cierre.id, 'fiscal')" />
                  </template>
                ...
                <td style="max-width: 200px; line-height: 1.15; white-space: break-spaces">
                  <template v-if="cierre.notas?.length">
                    ...
                    <a class="block q-my-xs" href="javascript:void(0)"
                      @click="openDialogEditarObservacionCierre(cierre)">
    ...
    <q-dialog allow-focus-outside v-model="showModalEditarLineaRecibo" class="j-dialog j-dialog-lg">
      <q-card>
        ...
        <q-card-section>
          <q-input type="textarea" outlined autogrow dense v-model="agregarEditarLineaRecibo.descripcion"
            label="Descripción" />
          ...
          <template v-if="agregarEditarLineaRecibo.ubicaciones?.length">
            ...
            <template v-for="ubicacion in agregarEditarLineaRecibo.ubicaciones">
              <div class="row q-col-gutter-sm q-mb-sm">
                ...
                <div class="col">
                  <q-input dense type="date" v-model="ubicacion.pagado_hasta" label="Pagado hasta" outlined
                    clearable></q-input>
                </div>
              </div>
            </template>
        ...
        <q-card-actions align="right">
          ...
          <q-btn label="Guardar" icon="credit_card" color="primary" @click="handleSubmitEditarLineaRecibo"
            :loading="isLoadingEditarLineaRecibo" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog allow-focus-outside v-model="showDialogBuscarPagos" class="j-dialog j-dialog-lg">
      <q-card>
        ...
        <q-card-section>
          <q-input dense outlined v-model="buscarReferenciaPago" debounce="300" label="Buscar referencia de pago"
            class="q-mb-md" clearable @update:model-value="val => handleBuscarReferenciaPago(val)" />
          <q-separator class="q-my-md" />
          <q-markup-table flat separator="cell" dense v-if="buscarReferenciaPago">
            ...
            <tbody>
              <tr v-for="pago in pagosEncontrados" :key="pago.id">
                <td class="text-center"><a href="javascript:void(0)" class="text-weight-bold text-green-9"
                    @click="openDialogAgregarPago(pago.transaccion_id)">{{ pago.num_transaccion }}</a></td>
                <td class="text-center">{{ pago.fecha_emision.substr(0, 10) }}</td>
                <td class="text-center">{{monedas.find(m => m.id == pago.moneda_id)?.simbolo}} {{ $dinero(pago.monto)
                }}
                </td>
          ...
          <template v-else>
            <div class="text-center q-py-xl bg-green-1 text-grey-7"
              style="border: 1px dashed var(--q-primary); border-radius: 5px">
            ...
            <tbody>
              <tr v-for="pago in transaccionData.pagos" :key="pago.id">
                <td>
                  ...
                  <q-btn size="sm" dense color="primary" icon="edit"
                    @click="showDialogEditarPago = true; dialogEditarPagoData = pago" />
                </td>
    ...
    <q-dialog allow-focus-outside v-model="showDialogEditarObservacionCierre" class="j-dialog j-dialog-lg">
      <q-card v-if="dialogObservacionesCierreData.id">
        <q-card-section>
          <div class="text-h6">{{ dialogObservacionesCierreData.nombre_caja }} ({{
            dialogObservacionesCierreData.created_at.substr(0, 10) }})</div>
        </q-card-section>
        <q-card-section>
          <div class="row q-col-gutter-md q-mb-sm" v-for="(nota, index) in dialogObservacionesCierreData.notas"
            :key="'nota-' + index">
            <div class="col-auto" style="display: flex;align-items: center;">
              <q-btn color="negative" icon="delete" size="sm"
                @click="dialogObservacionesCierreData.notas.splice(index, 1)"
                :disable="dialogObservacionesCierreData.notas.length == 1"></q-btn>
            </div>
            <div class="col">
          ...
          <div class="text-center q-mt-md">
            <q-btn color="primary" label="Agregar observación" icon="add"
              @click="dialogObservacionesCierreData.notas.push('')" />
          </div>
        </q-card-section>
        <q-separator></q-separator>
        <q-card-actions align="right">
          <q-btn color="primary" type="submit" label="Guardar" icon="save"
            @click="handleEditarObservacionesCierre(dialogObservacionesCierreData.id, dialogObservacionesCierreData.notas)" />
        </q-card-actions>
      </q-card>
    </q-dialog>
      ...
      <q-card>
        ...
        <q-card-section>
          <div class="row q-col-gutter-sm">
            <div class="col-6">
              <q-input type="number" step="0.01" v-model="dialogEditarPagoData.monto" label="Monto" clearable outlined
                dense />
            </div>
            <div class="col-6">
              <q-input type="number" step="0.01" v-model="dialogEditarPagoData.referencia" label="Referencia" clearable
                outlined dense />
            </div>
            <div class="col-3">
              <q-select v-model="dialogEditarPagoData.metodo_pago_id"
                :options="metodosPago.map(m => { return { label: m.metodo, value: m.id } })" label="Método de pago"
                clearable outlined dense map-options emit-value />
            </div>
            <div class="col-6">
            ...
            <div class="col-3">
              <q-input type="number" step="0.01" v-model="dialogEditarPagoData.tasa_ref" label="Tasa" clearable outlined
                dense />
            </div>
          </div>
        </q-card-section>
        <q-separator></q-separator>
        <q-card-actions align="right">
          <q-btn color="primary" type="submit" label="Guardar" icon="save" @click="handleEditarPago()"
            :loading="isLoadingEditarLineaRecibo" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog allow-focus-outside v-model="showDialogReporteGeneral" class="j-dialog j-dialog-full">
      <q-card>
        ...
        <q-card-section class="q-pa-md">
          <!-- SECCIÓN DE FILTROS -->
          <q-expansion-item icon="filter_list" label="Filtros avanzados" header-class="bg-primary text-white"
            class="q-mb-md">
              ...
              <q-card-section class="q-pa-md">
                <div class="row q-col-gutter-md">
                  ...
                  <div class="col-12 col-md-3">
                    ...
                    <div class="row q-col-gutter-md">
                      <div class="col-6">
                        <q-input dense outlined type="date" v-model="reporteFiltersActive.vigenciaDesde" label="Desde"
                          clearable />
                      </div>
                      <div class="col-6">
                        <q-input dense outlined type="date" v-model="reporteFiltersActive.vigenciaHasta" label="Hasta"
                          clearable />
                      </div>
                    </div>
                  ...
                  <div class="col-12 col-md-3">
                    ...
                    <div class="row q-col-gutter-md">
                      <div class="col-6">
                        <q-input dense outlined type="date" v-model="reporteFiltersActive.ultimoReciboDesde"
                          label="Desde" clearable />
                      </div>
                      <div class="col-6">
                        <q-input dense outlined type="date" v-model="reporteFiltersActive.ultimoReciboHasta"
                  ...
                  <div class="col-12 col-md-3">
                    ...
                    <q-select dense outlined v-model="reporteFiltersActive.estado" :options="['Activo', 'Inactivo']"
                      emit-value map-options clearable label="Seleccionar estado" />
                  </div>
                  ...
                  <div class="col-12 col-md-3">
                    ...
                    <div class="row q-col-gutter-sm">
                      <div class="col-6">
                        <q-select dense outlined v-model="reporteFiltersActive.moraOperador" :options="[
                      ...
                      <div class="col-6">
                        <q-input dense outlined type="number" v-model="reporteFiltersActive.moraCantidad" label="Número"
                          clearable />
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="col-12 col-md-3">
                    ...
                    <div class="row q-col-gutter-sm">
                      <div class="col-6">
                        <q-select dense outlined v-model="reporteFiltersActive.ubicacionesOperador" :options="[
                          { label: 'Igual', value: 'igual' },
                          { label: 'Mayor que', value: 'mayor' },
                          { label: 'Menor que', value: 'menor' }
                        ]" emit-value map-options />
                      ...
                      <div class="col-6">
                        <q-input dense outlined type="number" v-model="reporteFiltersActive.ubicacionesCantidad"
                          label="Número" clearable />
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="col-12 col-md-3">
                    ...
                    <div class="row q-col-gutter-sm">
                      <div class="col-6">
                        <q-select dense outlined v-model="reporteFiltersActive.deudaOperador" :options="[
                          { label: 'Igual', value: 'igual' },
                          { label: 'Mayor que', value: 'mayor' },
                          { label: 'Menor que', value: 'menor' }
                        ]" emit-value map-options />
                      ...
                      <div class="col-6">
                        <q-input dense outlined type="number" v-model="reporteFiltersActive.deudaCantidad" label="Monto"
                          clearable />
                      </div>
                    </div>
                  </div>
          ...
          <q-expansion-item icon="summarize" label="Resumen de estadísticas" header-class="bg-info text-white"
            class="q-mb-md">
            <q-card class="bg-blue-1">
              <q-card-section class="q-pa-md">
                <div class="row q-col-gutter-lg">
                  ...
                  <div class="col-12 col-md-4 col-lg">
                    <div class="text-center">
                      <div class="text-h6 text-positive text-weight-bold q-py-sm">{{ reporteSummary.clientesVigentes }}
                        ({{
                          reporteSummary.porcentajeClientesVigentes.toFixed(2) }}%)</div>
                  ...
                  <div class="col-12 col-md-4 col-lg">
                    <div class="text-center">
                      <div class="text-h6 text-red text-weight-bold q-py-sm">{{ reporteSummary.clientesEnMora }} ({{
                        reporteSummary.porcentajeClientesEnMora.toFixed(2) }}%)</div>
                  ...
                  <div class="col-12 col-md-4 col-lg">
                    <div class="text-center">
                      <div class="text-h6 text-negative text-weight-bold q-py-sm">{{ reporteSummary.clientesSuspendidos
                      }}
                      </div>
                  ...
                  <div class="col-12 col-md-6 col-lg">
                    <div class="text-center">
                      <div class="text-h6 text-red-9 text-weight-bold q-py-sm">${{ $dinero(reporteSummary.deudaActivos)
                      }}
                      </div>
                  ...
                  <div class="col-12 col-md-6 col-lg">
                    <div class="text-center">
                      <div class="text-h6 text-red-9 text-weight-bold q-py-sm">${{ $dinero(reporteSummary.deudaTotal) }}
                      </div>
          ...
          <q-virtual-scroll dense type="table" ref="reporteTableRef" style="max-height: 60vh"
            :virtual-scroll-item-size="48" :virtual-scroll-sticky-size-start="48" :virtual-scroll-sticky-size-end="32"
                ...
                <tr>
                  ...
                  <th style="cursor: pointer; user-select: none" @click="handleSetReporteSort('nombre_completo')">
                    ...
                    <q-icon name="arrow_upward" size="xs"
                      v-if="reporteSort.column === 'nombre_completo' && reporteSort.type === 'ASC'" />
                    <q-icon name="arrow_downward" size="xs"
                      v-if="reporteSort.column === 'nombre_completo' && reporteSort.type === 'DESC'" />
                  </th>
                  <th style="cursor: pointer; user-select: none" @click="handleSetReporteSort('codigo_seccion')">
                    Ubicación
                    <q-icon name="arrow_upward" size="xs"
                      v-if="reporteSort.column === 'codigo_seccion' && reporteSort.type === 'ASC'" />
                    <q-icon name="arrow_downward" size="xs"
                      v-if="reporteSort.column === 'codigo_seccion' && reporteSort.type === 'DESC'" />
                  </th>
                  <th style="cursor: pointer; user-select: none" @click="handleSetReporteSort('num_contrato')">
                    Contrato
                    <q-icon name="arrow_upward" size="xs"
                      v-if="reporteSort.column === 'num_contrato' && reporteSort.type === 'ASC'" />
                    <q-icon name="arrow_downward" size="xs"
                      v-if="reporteSort.column === 'num_contrato' && reporteSort.type === 'DESC'" />
                  </th>
                  <th style="cursor: pointer; user-select: none" @click="handleSetReporteSort('estatus')">
                    Estado
                    <q-icon name="arrow_upward" size="xs"
                      v-if="reporteSort.column === 'estatus' && reporteSort.type === 'ASC'" />
                    <q-icon name="arrow_downward" size="xs"
                      v-if="reporteSort.column === 'estatus' && reporteSort.type === 'DESC'" />
                  </th>
                  <th style="cursor: pointer; user-select: none" @click="handleSetReporteSort('vigente_hasta')">
                    Vigente hasta
                    <q-icon name="arrow_upward" size="xs"
                      v-if="reporteSort.column === 'vigente_hasta' && reporteSort.type === 'ASC'" />
                    <q-icon name="arrow_downward" size="xs"
                      v-if="reporteSort.column === 'vigente_hasta' && reporteSort.type === 'DESC'" />
                  </th>
                  <th style="cursor: pointer; user-select: none" @click="handleSetReporteSort('meses')">
                    Meses de mora
                    <q-icon name="arrow_upward" size="xs"
                      v-if="reporteSort.column === 'meses' && reporteSort.type === 'ASC'" />
                    <q-icon name="arrow_downward" size="xs"
                      v-if="reporteSort.column === 'meses' && reporteSort.type === 'DESC'" />
                  </th>
                  <th style="cursor: pointer; user-select: none" @click="handleSetReporteSort('ubicaciones')">
                    Ubicaciones
                    <q-icon name="arrow_upward" size="xs"
                      v-if="reporteSort.column === 'ubicaciones' && reporteSort.type === 'ASC'" />
                    <q-icon name="arrow_downward" size="xs"
                      v-if="reporteSort.column === 'ubicaciones' && reporteSort.type === 'DESC'" />
                  </th>
                  <th style="cursor: pointer; user-select: none" @click="handleSetReporteSort('fecha_ultimo_recibo')">
                    Último recibo
                    <q-icon name="arrow_upward" size="xs"
                      v-if="reporteSort.column === 'fecha_ultimo_recibo' && reporteSort.type === 'ASC'" />
                    <q-icon name="arrow_downward" size="xs"
                      v-if="reporteSort.column === 'fecha_ultimo_recibo' && reporteSort.type === 'DESC'" />
                  </th>
                  <th style="cursor: pointer; user-select: none" @click="handleSetReporteSort('total_pagar')">
                    Total deuda
                    <q-icon name="arrow_upward" size="xs"
                      v-if="reporteSort.column === 'total_pagar' && reporteSort.type === 'ASC'" />
                    <q-icon name="arrow_downward" size="xs"
                      v-if="reporteSort.column === 'total_pagar' && reporteSort.type === 'DESC'" />
                  </th>
                </tr>
              </thead>
            ...
            <template v-slot="{ item: row, index }">
              <tr :key="index">
                ...
                <!-- Estado -->
                <td>
                  <q-badge :color="row.estatus === 'Activo' ? 'positive' : 'negative'" :label="row.estatus"
                ...
                <td>
                  <template v-if="row.vigente_hasta">
                    <div :class="new Date(row.vigente_hasta) < new Date() ? 'text-red-9 text-weight-bold' : ''">
        ...
        <q-card-actions align="right" class="q-pa-md">
          <q-btn color="primary" label="Descargar PDF" icon="picture_as_pdf" @click="handleDescargarReportePdf()"
